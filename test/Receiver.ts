...
describe("Receiver Contract", () => {
    ...
    const getGasCost = async (tokenContractErc20: ERC20, amount: BigNumber) => {
        ...
        const txGas = await tokenContractErc20
            .connect(account4)
    ...
    beforeEach(async () => {
        [account1, account2, account3, account4, relayer] =
            await ethers.getSigners();
        receiver = <Receiver>(
            await (await ethers.getContractFactory("Receiver"))
                .connect(relayer)
                .deploy(relayer.address, name, version)
        );
        tokenContractErc20 = <ERC20>(
            await (
                await ethers.getContractFactory("MockErc20")
            ).deploy("Automata", "ATA", parseEther("1000000"))
        );
    });
    ...
    it("should execute  meta transactions for erc20 transferFrom", async () => {
        ...
        expect(await tokenContractErc20.balanceOf(account2.address)).to.equal(
            0
        );
        ...
        await expect(
            receiver.connect(account1).batch(callData, callParams)
        ).to.be.revertedWith("can only be executed by the meta tx processor");
        ...
        expect(
            await tokenContractErc20.balanceOf(account2.address)
        ).to.be.equal(amount);
    });

    it("should fail for insufficient token , Account has 0 tokens", async () => {
        const revertReason = utils.toUtf8Bytes(
            "ERC20: transfer amount exceeds balance"
        );
        const encodedRevertReason = utils.defaultAbiCoder.encode(
            ["bytes"],
            [revertReason]
        );
        ...
        const revertReasonInBytes =
            errorSignatureSelector + encodedRevertReason.slice(2);
        const amount = parseEther("100");
        const batchId = 0;
        const batchNonce = 1;
        const chainId = network.config.chainId;
        ...
        expect(await tokenContractErc20.balanceOf(account3.address)).to.equal(
            0
        );

        await tokenContractErc20
            .connect(account2)
            .approve(receiver.address, amount);
        ...
        expect(
            await tokenContractErc20.balanceOf(account3.address)
        ).to.be.equal(0);
    });
    ...
    it("should fail for higher txgas or lowerTxGas and fail for past time expiry", async () => {
        ...
        await expect(
            receiver.connect(relayer).batch(callData, callParams)
        ).to.not.emit(receiver, "MetaTx");
        expect(
            await tokenContractErc20.balanceOf(account2.address)
        ).to.be.equal(0);
        ...
        const signature_LowerGas = await account1._signTypedData(
            domain,
            types,
            value_LowerGas
        );
        ...
        await expect(
            receiver
                .connect(relayer)
                .batch(callData_LowerGas, callParams__LowerGas)
        ).to.not.emit(receiver, "MetaTx");
        ...
        const signature_PastExpiry = await account1._signTypedData(
            domain,
            types,
            value_PastExpiry
        );
        ...
        await expect(
            receiver
                .connect(relayer)
                .batch(callData_PastExpiry, callParams__PastExpiry)
        ).to.not.emit(receiver, "MetaTx");
    });

    it("should execute for multiple token ", async () => {
        const tokenContractErc20_2 = <ERC20>(
            await (
                await ethers.getContractFactory("MockErc20")
            ).deploy("DAI", "DAI", parseEther("1000000"))
        );
        ...
        expect(await tokenContractErc20_2.balanceOf(account2.address)).to.equal(
            0
        );
        ...
        expect(
            await tokenContractErc20_2.balanceOf(account2.address)
        ).to.be.equal(amount);
    });
    ...
    it("should fail for same BatchNonce/maximum gas usage/Invalid signatures", async () => {
        ...
        await expect(
            receiver.connect(relayer).batch(callData, [])
        ...
        await expect(
            receiver.connect(relayer).batch(To_Changed_callData, callParams)
        ).to.not.emit(receiver, "MetaTx");
        ...
        await expect(
            receiver.connect(relayer).batch(callData, Amount_Changed_callParams)
        ).to.not.emit(receiver, "MetaTx");
        ...
        await expect(
            receiver.connect(relayer).batch(callData, Token_Changed_callParams)
        ).to.not.emit(receiver, "MetaTx");
        ...
        await expect(
            receiver.connect(relayer).batch(callData, Nonce_Changed_callParams)
        ).to.not.emit(receiver, "MetaTx");
        ...
        await expect(
            receiver.connect(relayer).batch(callData, TxGas_Changed_callParams)
        ).to.not.emit(receiver, "MetaTx");
        ...
        await expect(
            receiver.connect(account4).batch(callData, callParams)
        ).to.not.emit(receiver, "MetaTx");
    });
});
